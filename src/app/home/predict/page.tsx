      ...
      <div className="predict-body">
        ...
        <div className="predict-stock-info">
          ...
          <div className="psi-right">
            ...
            <div className="psi-prev">전일 종가 73,880 · <span className="change-up">+920 (+1.24%)</span></div>
          </div>
        </div>

        <div className="chart-area">
          <div className="chart-label"><span>5일 주가 흐름</span><span>03.07 — 03.13</span></div>
        ...
        <div className="prediction-panel">
          ...
          <div className="input-group">
            ...
            <div className="price-input-wrapper">
              ...
              <input className="price-input" type="number" placeholder="75000" value={price} onChange={e => setPrice(e.target.value)} />
            </div>
            <div className="quick-buttons">
              {[73000, 74500, 75000, 76000].map(v => (
                <button key={v} className="quick-btn" onClick={() => setPrice(String(v))}>{v.toLocaleString()}</button>
              ))}
            </div>
          </div>
          <div className="confidence-group">
            ...
            <input type="range" className="confidence-slider" min="0" max="100" value={confidence} onChange={e => setConfidence(Number(e.target.value))} />
          </div>
          <div className="submit-row">
            <button className="btn-cancel" onClick={() => router.push('/home')}>취소</button>
            <button className="btn-submit" onClick={() => router.push('/home/result')}>예측 제출하기 →</button>
          </div>
        </div>
      </div>
    </div>
